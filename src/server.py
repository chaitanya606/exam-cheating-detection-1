...
def encode_frame_to_base64(frame):
    """Encode a BGR OpenCV frame to base64 JPEG string for the browser."""
...
def main(socketio=None):
    """
    If socketio is None  -> behave like before (desktop window).
    If socketio is given -> stream frames & events over websockets, no cv2 window.
    """
    config = load_config()
    ...
    try:
        ...
        while True:
            ...
            if violation_type:
                ...
                # ALSO send violation info to browser if using websockets
            ...
            # Display overlays and record
            display_detection_results(frame, results)
            video_recorder.record_frame(frame)
            
            # --- Desktop vs Web mode ---
            if socketio is None:
                # Old behavior: show local OpenCV window
                cv2.imshow('Exam Proctoring', frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
            else:
                # New behavior: send frame & results to web client
                ...
                # allow SocketIO to process other events
                socketio.sleep(0)
...
if __name__ == '__main__':
    # Running server.py directly still opens the desktop preview (no websockets)
    main()
